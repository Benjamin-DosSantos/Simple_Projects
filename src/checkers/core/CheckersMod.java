 ...
 *	Project Description: The Checker program is intended to make a checker
...
public class CheckersMod extends Applet{
	...
	public void paint(Graphics g){
		...
		int sizeofboard = 26; 	/** The size of the board to be generated in an NxN square **/
             ...
             for (int col = 0;  col < sizeofboard;  col++) {	/** Does the inner Code for the size of the board, adds one to the row and resets the col **/
                ...
                if ( (row % 2) == (col % 2) )	/** Checks the value of the row and col and if they are the same they draw red if not then drae black **/
                   ...
                   g.fillRect(x, y, squaresize, squaresize);	// Fills a 20x20 square(Because width and height are the same) at the x and y in the next space
                   	...
                   	if(g.getColor() == Color.RED && row < 3){ 	/** Checks the color of the square if it is red and it is rows 1-3 it places a player_1 **/
                   
                   		g.setColor(Color.GREEN);	/** Sets the player 1 color **/
                   		g.fillOval(x + 2, y + 2, playersize, playersize); 	/** Sets the player position **/
                   
                   		}		// End of if g.color
                   
                   if(g.getColor() == Color.RED && row > (sizeofboard - 4)){	/** Checks the color of the square if it is red and it is rows (sizeofboard - 4) - sizeofboard it places a player_1 **/
                	   
                	   g.setColor(Color.WHITE); 	/** Sets the Player_2 Color **/
                       g.fillOval(x + 2, y + 2, playersize, playersize);	/** Sets the player position **/
